 ...
 * 
 * @return {(AllianceMembership|null)} - The `AllianceMembership` enum value matching the input. Returns `null`, if the provided `fleetRank` doesn't match an enum value. 
 */
 ...
 * 
 * @return {AllianceMembership} - A `Date` object representing a specific point in time since PSS started, if `secondsSincePssStart` is truthy. Else, `null`.
 */
...
/**
 * Takes an `url` and a dictionary of query parameters and turns them into a full URL.
 *
...
/**
 * Takes some parameters and adds them to a dictionary, if they're defined.
 *
 ...
 * 
 * @return {(string|null)} - A `Interval` converted to its string representation, if the input value is of type `Interval`. The input, if the input is of type `string`. Else, `null`.
 */
 ...
 * 
 * @return {(string|null)} - A string representing the UTC portion of the provided `datetime`, if it's defined. Else, `null`. The resulting string resembles an ISO-8601 string without the 'T' separator nor timezone information.
 */
